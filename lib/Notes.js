...
class Notes {
  ...
  init() {
    return scan(this.dir).then((data) => this.data = data).catch(console.log);
  }
  ...
  updateNote(path, note) {
    ...
    var result = this._getCategory(this.data, this._split(dir), 0).files[file];

    parseMeta(result, note.content);

    result.updated_at = new Date();

    return writeFile(p.join(this.dir, result.file), note.content).then(() => {
